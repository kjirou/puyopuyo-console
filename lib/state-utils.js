...
const generateFallingPuyoSquares = (state) => {
  return state.game.gameLoop.divisionId === GAME_LOOP_DIVISION_IDS.FALLING_PUYO_OPERATION_RUNNING
...
const calculateNextGameLoopDivision = (gameLoopState) => {
  const nextDivisionId = {
    [GAME_LOOP_DIVISION_IDS.FALLING_PUYO_OPERATION_START]: GAME_LOOP_DIVISION_IDS.FALLING_PUYO_OPERATION_RUNNING,
  }[gameLoopState.divisionId] || gameLoopState.divisionId;

  return icepick.assocIn(gameLoopState, ['divisionId'], nextDivisionId);
};
...
const createInitialState = () => {
  const state = {
    ...
    game: {
      gameLoop: {
        divisionId: GAME_LOOP_DIVISION_IDS.FALLING_PUYO_OPERATION_START,
      },
    },
    gameLoopId: 0,
    gameTime: 0,
...
const gameLoopDivisionHandlers = {
  [GAME_LOOP_DIVISION_IDS.FALLING_PUYO_OPERATION_RUNNING]: (state) => {
    // Apply pad operation
    ...
    // Apply free fall
    if (state.fallingPuyo.lastMovedAt + FALLING_PUYO_FREE_FALL_INTERVAL <= state.gameTime) {
      const nextCoordinate = {
      ...
      if (
        ...
        state = icepick.assocIn(state, ['fallingPuyo', 'coordinate'], nextCoordinate);
        state = icepick.assocIn(state, ['fallingPuyo', 'lastMovedAt'], state.gameTime);
      }
    }
...
const calculateNextGameLoopState = (previousState) => {
  ...
  state = icepick.assocIn(state, ['game', 'gameLoop'], calculateNextGameLoopDivision(state.game.gameLoop));
  ...
  state = icepick.assocIn(state, ['gameLoopId'], state.gameLoopId + 1);
  state = icepick.assocIn(state, ['gameTime'], state.gameTime + PARAMETERS.GAME_LOOP_INTERVAL);
