...
class Store {
  ...
  static _findMatchedSquaresSets(puyopuyoFieldSquareMatrix) {
    const scanAroundRecursively = (matchedSquares, squareMatrix, ignoreSquares) => {
      let nextMatchedSquares = matchedSquares.slice();
      const currentSquare = matchedSquares[matchedSquares.length - 1];

      Object.keys(DIRECTIONS).forEach(key => {
        const direction = DIRECTIONS[key];
        const delta = DIRECTIONS_TO_COORDINATE_DELTAS[direction];
        const targetSquare = Store._getSquare(
          puyopuyoFieldSquareMatrix,
          currentSquare.rowIndex + delta.rowIndex,
          currentSquare.columnIndex + delta.columnIndex
        );

        if (
          !targetSquare ||
          matchedSquares.indexOf(targetSquare) !== -1 ||
          ignoreSquares.indexOf(targetSquare) !== -1 ||
          targetSquare.colorType !== currentSquare.colorType
        ) {
          return;
        }
        nextMatchedSquares.push(targetSquare);

        nextMatchedSquares =
          scanAroundRecursively(nextMatchedSquares, squareMatrix, ignoreSquares);
      });

      return nextMatchedSquares;
    };

    const matchedSquaresSets = [];

    flatten(puyopuyoFieldSquareMatrix, 1).forEach(square => {
      const finishedSquares = flatten(matchedSquaresSets, 1);

      if (
        PUYOPUYO_CONTROLLABLE_COLORS.indexOf(square.colorType) === -1 ||
        finishedSquares.indexOf(square) !== -1
      ) {
        return;
      }

      const matched = scanAroundRecursively([square], puyopuyoFieldSquareMatrix, finishedSquares);
      matched.sort((a, b) => {
      ...
      matchedSquaresSets.push(matched);
    });

    return matchedSquaresSets
