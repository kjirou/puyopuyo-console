...
const gameLoopDivisionHandlers = {
  [GAME_LOOP_DIVISION_IDS.FALLING_PUYO_OPERATION_START]: (state) => {
    state = icepick.assocIn(state, ['fallingPuyo'], createFallingPuyoState());
    state = icepick.assocIn(
      state, ['game', 'gameLoop', 'divisionId'], GAME_LOOP_DIVISION_IDS.FALLING_PUYO_OPERATION_RUNNING);
  ...
  [GAME_LOOP_DIVISION_IDS.FALLING_PUYO_OPERATION_RUNNING]: (state) => {
    ...
    if (state.fallingPuyo.lastMovedAt + FALLING_PUYO_FREE_FALL_INTERVAL <= state.game.gameTime) {
      ...
      if (
        ...
        state = icepick.assocIn(
          state, ['game', 'gameLoop', 'divisionId'], GAME_LOOP_DIVISION_IDS.FALLING_PUYO_OPERATION_START);
      } else {
        state = icepick.assocIn(state, ['fallingPuyo', 'coordinate'], nextCoordinate);
