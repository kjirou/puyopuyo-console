...
const gameLoopDivisionHandlers = {
  ...
  [DIDS.NATURAL_DROPS_INIT]: (state) => {
    ...
    state = icepick.assocIn(state, ['game', 'gameLoop', 'divisionId'], DIDS.NATURAL_DROPS_RUNNING);
  ...
  [DIDS.MATCHED_PUYOS_DISAPPEARANCE_INIT]: (state) => {
    ...
    state = icepick.assocIn(state, ['game', 'gameLoop', 'divisionId'], DIDS.MATCHED_PUYOS_DISAPPEARANCE_RUNNING);
